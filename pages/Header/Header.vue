...
import logo from "../../assest/Images/logos/logo.png";
...
<template>
  <header
    class="bg-white py-8 relative z-50 transition-all duration-1000"
    :class="{ 'sticky top-0 w-full shadow-lg py-4 animate-fadeInDown': isSticky }"
  >
    <div class="container">
      <div class="flex items-center justify-between gap-4">
        ...
        <nav class="hidden lg:flex gap-6">
          <NuxtLink to="/" class="text-lg font-medium hover:text-primarycolor transition">Home</NuxtLink>
          <NuxtLink to="/services" class="text-lg font-medium hover:text-primarycolor transition">Services</NuxtLink>
          <NuxtLink to="/technology" class="text-lg font-medium hover:text-primarycolor transition">Technology</NuxtLink>
          <NuxtLink to="/portfolio" class="text-lg font-medium hover:text-primarycolor transition">Portfolio</NuxtLink>
          <NuxtLink to="/company" class="text-lg font-medium hover:text-primarycolor transition">Company</NuxtLink>
          <NuxtLink to="/career" class="text-lg font-medium hover:text-primarycolor transition">Career</NuxtLink>
          <NuxtLink to="/contact" class="lg:hidden">Contact Us</NuxtLink>
        </nav>

        <!-- Mobile Menu Overlay -->
        <div
          class="fixed top-0 left-0 w-full h-screen bg-black bg-opacity-50 z-40 transition-opacity duration-300 lg:hidden"
        ...
        <!-- Mobile Navigation -->
        <nav
          class="fixed top-0 left-0 w-[320px] bg-white shadow-lg h-screen z-50 transition-transform duration-300 lg:hidden"
        ...
        <NuxtLink
          to="/contact"
          class="hidden lg:flex items-center gap-2 bg-primarycolor rounded-full overflow-hidden text-white relative h-[57px] w-[182px] border-0 text-[18px] font-medium text-custom-white transition-all duration-500 group"
          aria-label="Contact Us"
        >
          <span class="absolute bg-secondarycolor h-full p-[16px_22px] rounded-[80px_0_80px_80px] text-center flex items-center justify-center">
            Contact Us
          </span>
        </NuxtLink>
      </div>
